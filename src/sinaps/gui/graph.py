...
class SimuView:
    ...
    def field(self, df, time=None, res=None):  # TODO make it a static method
        ...
        dfi = interpolate.interp1d(
            self.simu.N.indexV_flat(), df.values, fill_value="extrapolate"
        )
        if res is None:
            res = max(max(self.simu.N.indexV_flat()) / 1000, 1)
        y = np.arange(0, max(self.simu.N.indexV_flat()) + res, res)
        return rasterize(hv.QuadMesh((df.index.values, y, dfi(y).T))).opts(
            xlabel="Time (ms)",
            ylabel="Position (μm)",
            width=600,
            height=600,
    ...
    def C_field(self, ion, **kwargs):
        return self.field(self.simu.C[ion], **kwargs).opts(
            title="{} Concentration".format(ion)
        )

    def I_field(self, ch_cls, **kwargs):
        return self.field(self.simu.current(ch_cls), **kwargs).opts(
            title="{} Current".format(ch_cls.__name__)
        )
